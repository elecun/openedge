...
class DKM_DX3000 : public oe::device::controller {
    ...
    public:
        ...
        //DKM_DX3000(int id = 1); //for native
...
//for native (only generate packet)
class DKM_DX3000_NATIVE : public oe::device::controller {
    public:
        enum class DIRECTION : int {
            NONE = 0,
            CW,
        ...
        DKM_DX3000_NATIVE(int id=1);
        virtual ~DKM_DX3000_NATIVE();
        ...
        //service api
        int move(unsigned char* dest, DIRECTION dir);    //generate motor move frame
        int stop(unsigned char* dest);    //stop motor
        bool set_parameter(PARAMETER opt, variant<int, double> param);
        int set_rpm(unsigned char* dest, unsigned short rpm);
        unsigned short get_rpm(unsigned char* dest);
        unsigned short get_status(unsigned char* dest);
