...
int system::getProcIdByName(const std::string &procName) {
    int pid = -1;
    DIR *dp = opendir("/proc");
    if (dp != nullptr) {
        struct dirent *dirp;
...
int system::killProcessById(int pid, const std::string &procName) {
    if (pid == -1) {
...
uint64_t system::getFreeDiskSpace(std::string absoluteFilePath) {
    struct statvfs buf;
...
uint64_t system::userAvailableFreeSpace() {
    struct statvfs stat;
...
std::string system::getOSVersion_Signature(void) {
...
std::string system::getOsVersionString(void) {
    std::ifstream versionFile;
...
bool system::isDeviceOnline(std::string address) {
...
uint32_t system::getNumOfThreadsByThisProcess() {
    uint32_t Threads = 0;
...
uint32_t system::getNumOfThreadsByPID(int Pid) {
    uint32_t Threads = 0;
    std::ifstream memoryFile;
...
std::string system::getIFaceMacAddress(std::string deviceName) {
