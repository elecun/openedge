...
netload::netload(std::string ethernetDataFileName, std::string ethName) : ethernetDataFile(std::move(ethernetDataFileName)), ethDev(std::move(ethName)) {
    this->initNetworkMonitor();
}


bool netload::isDeviceUp() const {
    return this->isDeviceAvailable;
}


std::string netload::getDeviceName() {
    return this->ethDev;
}

void netload::initNetworkMonitor() {
...
uint64_t netload::parseEthernetDevice() {
...
// std::list<std::string> netload::scanNetworkDevices(const std::string& ethernetDataFile) {
...
uint64_t netload::getTXBytesPerSecond() {
...
uint64_t netload::getRXBytesPerSecond() {
...
uint64_t netload::getRXBytesSinceStartup() {
    this->parseEthernetDevice();
...
uint64_t netload::getTXBytesSinceStartup() {
    this->parseEthernetDevice();
    uint64_t transceivedBytes = 0;
    try {
...
uint64_t netload::getBytesPerSecond() {
...
uint64_t netload::getBytesSinceStartup() {
    this->parseEthernetDevice();
    uint64_t transceivedBytes = 0;
    try {
        transceivedBytes = std::stoull(this->networkstatMap["RXbytes"], nullptr, 10);
...
std::string netload::getBytesPerSeceondString(uint64_t bytesPerSecond) {
    return getBytesString(bytesPerSecond) + "/s";
}

std::string netload::getBitsPerSeceondString(uint64_t bytesPerSecond) {
    return getBitsString(bytesPerSecond) + "/s";
}

std::string netload::getBytesString(uint64_t totalBytes) {
...
std::string netload::getBitsString(uint64_t totalBytes) {
