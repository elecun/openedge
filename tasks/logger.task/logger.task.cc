...
#define TRIGGER_THRESHOLD 1000 //1v
void loggerTask::execute(){
    static bool trigger = false;

    if(_fd>0){
        if(ioctl(_fd, I2C_SLAVE, 0x42)>=0){
            unsigned short value[3] = {0x0000, };
            value[0] = (unsigned short)i2c_smbus_read_word_data(_fd, 0x02);
            unsigned short trigger_value = (unsigned short)(value[0]>>8|value[0]<<8);
            
            
            if(trigger_value>TRIGGER_THRESHOLD && trigger==false){
                trigger = true;
            ...
            else if(trigger_value<TRIGGER_THRESHOLD && trigger==true) {
                trigger = false;
                
                //close file
                if(_logfile.is_open()){
                    _logfile.close();
                    spdlog::info("Close logfile");
                }
            }
            else if(trigger_value>TRIGGER_THRESHOLD && trigger==true){
                if(_logfile.is_open()){
